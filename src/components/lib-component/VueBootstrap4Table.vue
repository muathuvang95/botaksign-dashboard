    ...
    <div :class="{card:card_mode}">
        ...
        <div :class="{'card-body':card_mode}">
            <div :class='tableWrapperClasses' class="vbt-table-wrapper">
                ...
                <table class="table table-responsive" :class="tableClasses">
                    <thead>
                        <tr v-if="showToolsRow" class="vbt-table-tools">
                            <th :colspan="headerColSpan">
                                <div class="vbt-header-row no-gutters">
                                    <!-- global search text starts here -->
                                    <div class="input-group vbt-global-search" v-if="global_search.visibility">
                                        <div class="form-group has-clear-right" :class="global_search.class">
                                            <label>View orders under:</label>
                                            <div class="btn-toolbar" role="toolbar" aria-label="Toolbar with button groups">
                                                 <div class="input-group">
                                                    <SearchSpecialist :rows="rows" :config="config"></SearchSpecialist>
                                                </div>
                                                <div class="btn-group" role="group" aria-label="First group">
                                                    <button type="button" v-if="show_reset_button" class="vbt-reset-button" @click="updateBoard">
                                    ...
                                    <!-- <div class="col-md-8">
                                        <slot name="vbt-action-buttons">
                                            <div class="btn-group float-right" role="group" aria-label="Basic example">
                                                <button v-for="(action, key, index) in actions" :key="index" type="button" class="btn" :class="getActionButtonClass(action)" @click="emitActionEvent(action)">
                        ...
                        <tr>
                            <!-- <select-all-rows-check-box v-if="checkbox_rows" :all-rows-selected="allRowsSelected" :current-page-selection-count="currentPageSelectionCount" @select-all-row-checkbox="selectAllCheckbox" /> -->

                            <slot name="columns" :columns="vbt_columns">
                                <template v-for="(column, key, index) in vbt_columns">
                                    <th v-if="canShowColumn(column)" :key="index" v-on="isSortableColumn(column) ? { click: () => updateSortQuery(column) } : {}" class="vbt-column-header" :class="columnClasses(column)" v-bind:colspan="column.colspan">
                                        ...
                                        <template v-if="hasFilter(column)">
                                            <FilterOrder v-if="column.filter.type == 'FilterOrder'" :column="column" :searchSpecialistValue="searchSpecialistValue" @get-column="updateFilter"></FilterOrder>
                                            <MultiSearch v-if="column.filter.type == 'MultiSearch'" :column="column" :searchSpecialistValue="searchSpecialistValue"></MultiSearch>
                    ...
                    <tbody>
                        ...
                        <template v-for="(row, index) in vbt_rows">
                            ...
                            <!-- <row v-if="row.level==1" :key="index" :row="row" :columns="vbt_columns" :row-index="index" :checkbox-rows="checkbox_rows" :rows-selectable="rows_selectable" :selected-items="selected_items" :highlight-row-hover="highlight_row_hover" :highlight-row-hover-color="rowHighlightColor" :prop-row-classes="classes.row" :prop-cell-classes="classes.cell" :unique-id="uniqueId" @add-row="handleAddRow" @remove-row="handleRemoveRow"> -->
                            <row v-if="row.level==1" :key="index" :row="row" :columns="vbt_columns" :row-index="index" :rows-selectable="rows_selectable" :selected-items="selected_items" :highlight-row-hover="highlight_row_hover" :highlight-row-hover-color="rowHighlightColor" :prop-row-classes="classes.row" :prop-cell-classes="classes.cell" :unique-id="uniqueId" @add-row="handleAddRow" @remove-row="handleRemoveRow">
                            ...
                            <ChildTable :row="row" :rows="rows"></ChildTable>
                        </template>

                        <!-- empty row starts here -->
                        <tr v-show ="!vbt_rows.length">
                        ...
                        <tr v-if="showPaginationRow" class="footer-pagination-row">
                            <td :colspan="headerColSpan">
                                <div class="row vbt-pagination-row no-gutters">
                                    ...
                                    <div class="col-md-8">
                                        <div v-if="pagination">
                                            <Pagination :page.sync="page" :per_page.sync="per_page" :per_page_options="per_page_options" :total="rowCount" :num_of_visibile_pagination_buttons="num_of_visibile_pagination_buttons" :searchSpecialistValue="searchSpecialistValue">
...
<script>
    ...
    // import Row from "vue-bootstrap4-table/src/components/Row";
    import Row from "./Row";
    import CheckBox from "vue-bootstrap4-table/src/components/CheckBox";
    import SelectAllRowsCheckBox from "vue-bootstrap4-table/src/components/SelectAllRowsCheckBox";
    import SortIcon from "vue-bootstrap4-table/src/components/SortIcon";
    import Pagination from "./Pagination";
    import FilterOrder from "./FilterOrder";
    import MultiSearch from "./MultiSearch";
    import MultiSelect from "vue-bootstrap4-table/src/components/Filters/MultiSelect";
    ...
    import {
        EventBus
    } from 'vue-bootstrap4-table/src/event-bus.js';
    ...
    export default {
        ...
        mounted() {
            ...
            this.handleShiftKey();
            self.$root.$on('getSearchSpecialistValue', (value) => {
                self.searchSpecialistValue = value;
            })

            self.$root.$on('getTotalRowsAfterSearch', (totalRows) => {
                self.totalRowsCs = totalRows;
            }) 
            self.$root.$on('resetPagition', (status) => {
                if(status == true) {
                    jQuery('.pagination').children('li.page-item').eq(0).addClass('disabled');
                    jQuery('.pagination').children('li.page-item').removeClass('active');
                    jQuery('.pagination').children('li.page-item').eq(1).addClass('active');
                }
            })
            this.$root.$on('updatePage', function(items) {
                self.page = items;
            });
        },
        components: {
        ...
        methods: {
            ...
            filter(resetPage = true, isInit = false) {
                ...
                let res = filter(this.original_rows, (row) => {
                    ...
                    this.query.filters.some((filter, key) => {
                        if (filter.type === "simple") {
                            ...
                            let index = findIndex(this.vbt_columns, {
                                name: filter.name
                            });
                            if (index > -1) {
                ...
                self.$root.$on( 'updatePage' , function(index) {
                    self.page = index;
                });
            },
            ...
            updateBoard() {
                this.$root.$emit( 'refreshBoard' , true);
            },
