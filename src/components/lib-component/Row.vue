<template>
    ...
    <tr :data-id="rowId" ref="vbt_row" v-bind:style='{"background": (rowHiglighted) ? highlightRowHoverColor : ""}' :class='rowClasses' v-on="rowsSelectable ? { click: ($event) => handleRowSelect($event) } : {}">
        <CheckBox v-if="checkboxRows" :rowsSelectable="rowsSelectable" :row-selected="rowSelected" @add-row="addRow" @remove-row="removeRow" />

        <template v-for="(column, key, hindex) in columns">
            <td v-if="canShowColumn(column)" :key="hindex" :class="cellClasses(column , key)">
...
<script>
    ...
    import CheckBox from "vue-bootstrap4-table/src/components/CheckBox.vue";
    ...
    export default {
        ...
        methods: {
            ...
            cellClasses(column , index) {
                ...
                if(this.row.status == 'Cancelled' && index == 8 && this.row.date_time_out) {
                    classes += ' bg-red';
                }
